...
const TestHypothesis = () => {
    ...
    const generateChartData = () => {
        ...
        const fs_used_percent = 100.0 * ( 1 - 1.0 * fs_free / fs_total) - data_size_percent;
            ...
            <Row gutter={[8, 8]}>
                <Col span={12}>
                    <Row gutter={[16, 16]}>
                        ...
                        >
                            <div>
                                Доступно <span className={style.plate}>{bytesToGB(fsChartData.free_in_bytes)}</span> Гб  из <span className={style.plate}>{bytesToGB(fsChartData.total_in_bytes)}</span> Гб
                            </div>
                            ...
                            >
                                {chartData.map((entry, index) => (
                                    <Cell key={`cell-${index}`} fill={setupColor(entry.name)} />
                                ))}
                            </Pie>
                            <Tooltip />
                            </PieChart>
