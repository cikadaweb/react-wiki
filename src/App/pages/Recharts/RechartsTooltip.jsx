    ...
    if (entries && entries.length > 1) {
      for (let i = 1; i < entries.length; i++) {
        gdata.push({
          time: entries[i]['time'],
          mean: Math.max(
            (entries[i]['mean'] - entries[i - 1]['mean']) / newInterval,
            0,
          ),
        });
      }
    ...
    if (active) {
      return (
        <div className={style.chart__tooltip}>
          <div className="label">
            Время: {`${format(payload[0].payload.time, 'HH:mm:ss')}`}
          </div>
          <div className="label">
            Скорость: {`${payload[0].payload.mean.toFixed(2)}`}
          </div>
        </div>
      );
    }
    ...
    <div className="container">
      <ResponsiveContainer width={500} height={300}>
        <LineChart
          ...
          <Tooltip content={<CustomTooltip payload={chartData.data} />} />
